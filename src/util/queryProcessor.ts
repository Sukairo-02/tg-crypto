	...
	try {
		...
		switch (query[0]) {
			...
			default: {
				...
				if (!cCrypCache) {
					const res = (await cryptoRequest.getCurrency(cSymb)).data[
						cSymb
					]
				...
				return {
					msg: `${cCrypCache?.name} - ${date[0]} ${
						...
						cCrypCache?.volChange24h
					}</code>\nPercent change:\n\t1h - <code>${
						cCrypCache?.percentChange1h
					}%</code>\n\t24h - <code>${
						cCrypCache?.percentChange24h
					}%</code>\n\t7d - <code>${
						cCrypCache?.percentChange7d
					}%</code>\n\t30d - <code>${
						cCrypCache?.percentChange30d
					}%</code>\n\nMarket cap - <code>${
						cCrypCache?.marketCap
					}</code>\nDominance - <code>${
						cCrypCache?.marketCapDom
					}</code>\nFully diluted - <code>${
						cCrypCache?.fullyDiluted
					}</code>`,
					btn: {
						inline_keyboard: [
							[
